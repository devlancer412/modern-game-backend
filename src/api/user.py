...
from src.models import NFT, DWHistory, DepositMethod, Direct, NFTHistory, NFTNote, NFTType, Network, User, UserBalance
...
from src.utils.web3 import ETH_ERC1155_TRANSFER_TOPIC, compare_eth_address, erc1155_data_dispatch, get_current_gas_price, get_transaction_eth_value, get_transaction_nft_data, get_transaction_token_value, send_eth_stable_to, wait_transaction_receipt
...
class UserAPI(Function):
    ...
    def Bootstrap(self, app: FastAPI):
        ...
        async def deposit_eth_manually(tx_hash: str = Query(default = None, regex='0x[a-z0-9]{64}'), payload: TokenPayload = Depends(get_current_user_from_oauth), session:Session = Depends(get_db_session)) -> float:
          if tx_hash == None:
            raise HTTPException(
          ...
          fee = get_current_gas_price() * int(cfg.ETH_SWAP_FEE)
          tx = self.uniswap.make_trade(eth, eth_usdt, int(to_wei(tx_data['value'], 'wei') - fee))
        ...
        async def deposit_eth_stable_manually(tx_hash: str = Query(default = None, regex='0x[a-z0-9]{64}'), payload: TokenPayload = Depends(get_current_user_from_oauth), session:Session = Depends(get_db_session)) -> float:
          if tx_hash == None:
            raise HTTPException(
              status_code=status.HTTP_400_BAD_REQUEST,
              detail="You need to set hash"
          ...
          if not compare_eth_address(tx_data["contract"], eth_usdt) and not compare_eth_address(tx_data["contract"], eth_usdc):
            raise HTTPException(
              status_code=status.HTTP_400_BAD_REQUEST,
        ...
        async def deposit_eth_nft_manually(tx_hash: str = Query(default = None, regex='0x[a-z0-9]{64}'), payload: TokenPayload = Depends(get_current_user_from_oauth), session:Session = Depends(get_db_session)):
          if tx_hash == None:
            raise HTTPException(
              status_code=status.HTTP_400_BAD_REQUEST,
              detail="You need to set hash"
        ...
        async def withdraw_eth_eth(amount: float = Query(default=0), wallet: str = Query(default=None, regex='0x[A-Za-z0-9]{40}'), payload: TokenPayload = Depends(get_current_user_from_oauth), session:Session = Depends(get_db_session)) -> float:
          ...
          delta_balance = float(self.uniswap.get_price_output(eth_usdt, eth, amount*10**18) + fee)/10**6
        ...
        async def withdraw_eth(amount: float = Query(default=0), wallet: str = Query(default=None, regex='0x[A-Za-z0-9]{40}'), payload: TokenPayload = Depends(get_current_user_from_oauth), session:Session = Depends(get_db_session)) -> float:
          ...
          fee = get_current_gas_price() * int(cfg.ETH_SWAP_FEE)
          fee_usdt = float(self.uniswap.get_price_output(eth_usdc, eth, fee))/10**6
        ...
        async def withdraw_sol_sool(amount: float = Query(default=0), wallet: str = Query(default=None, regex='[A-Za-z0-9]{42-44}'), payload: TokenPayload = Depends(get_current_user_from_oauth), session:Session = Depends(get_db_session)) -> float:
          if amount == 0:
            raise HTTPException(
              status_code=status.HTTP_400_BAD_REQUEST,
              detail="You need to set amount more than 0"
