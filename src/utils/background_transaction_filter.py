...
from .web3 import compare_eth_address, erc1155_data_dispatch, uint256_to_address, web3_eth, solana_client
    ...
    for tx in transactions:
      ...
      for log in transfer_logs:
        ...
        if compare_eth_address(contract_address, cfg.ETH_USDT_ADDRESS) or compare_eth_address(contract_address, cfg.ETH_USDC_ADDRESS):
          to_wallet.is_using = False
          await deposited_eth_stabele_coin(coin=contract_address, wallet=to_wallet, amount=int(log.data, 16), hash=tx.hash)
          continue
...
async def handle_log(context):
  ...
  for instruction in instructions:
    ...
    if program == SOL_PROGRAM_ID:
      ...
      print("Sol transfer: {} -> {} : {}sol".format(accounts[instruction['accounts'][0]], accounts[instruction['accounts'][1]], float(amount)/10**9))
      continue
